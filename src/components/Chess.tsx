       ...
       <div className="flex flex-col gap-y-6 justify-center place-items-center h-full">
            ...
            <div className="relative grid grid-cols-8 border-8 border-spacing-10 border-amber-800">
                {cases.map((value, index) => (
                    <p key={index} id={chessPlace[index]} className={`hover:bg-red-200 flex justify-center place-items-center ${bgColor[index]} w-20 h-20`} onClick={() => caseAsClicked(index, cases, setCases, setBgColor, pieceSelect, setPieceSelect, idSelect, setIdSelect)}>
